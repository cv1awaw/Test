...
async def handle_private_message_for_group_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == SUPER_ADMIN_ID and user.id in pending_group_names:
        ...
        if group_name:
            ...
            await message.reply_text(f"âœ… Group name for `{g_id}` set to: *{group_name}*")
            logger.info(f"Group name for {g_id} set to {group_name} by SUPER_ADMIN {user.id}")
        else:
            await message.reply_text("âš ï¸ Group name cannot be empty. Please try `/group_add` again.")
            logger.warning(f"Empty group name received from SUPER_ADMIN {user.id} for group {g_id}")
    else:
        await message.reply_text("âš ï¸ No pending group to set name for.")
...
async def set_warnings_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id != SUPER_ADMIN_ID:
        await update.message.reply_text("âŒ You don't have permission to use this command.")
        logger.warning(f"Unauthorized access attempt to /set by user {user.id}")
        return
    args = context.args
    if len(args) != 2:
        await update.message.reply_text("âš ï¸ Usage: `/set <user_id> <number>`")
    ...
    try:
        ...
        await update.message.reply_text("âš ï¸ Both `user_id` and `number` must be integers.")
        logger.warning(f"Non-integer arguments provided to /set by SUPER_ADMIN {user.id}")
        return
    if new_warnings < 0:
        await update.message.reply_text("âš ï¸ Number of warnings cannot be negative.")
    ...
    await update.message.reply_text(f"âœ… Set `{new_warnings}` warnings for user ID `{target_user_id}`.")
...
async def tara_g_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 1:
        await update.message.reply_text("âš ï¸ Usage: `/tara_G <admin_id>`")
        logger.warning(f"Incorrect usage of /tara_G by SUPER_ADMIN {user.id}")
        return
    try:
        new_admin_id = int(context.args[0])
    except ValueError:
        await update.message.reply_text("âš ï¸ `admin_id` must be an integer.")
        logger.warning(f"Non-integer admin_id provided to /tara_G by SUPER_ADMIN {user.id}")
        return
    add_global_tara(new_admin_id)
    await update.message.reply_text(f"âœ… Added global TARA admin `{new_admin_id}`.")
...
async def tara_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id != SUPER_ADMIN_ID:
        ...
        return
    if len(context.args) != 1:
        await update.message.reply_text("âš ï¸ Usage: `/tara <tara_id>`")
    ...
    await update.message.reply_text(f"âœ… Added normal TARA `{tara_id}`.")
...
async def remove_normal_tara_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id != SUPER_ADMIN_ID:
        ...
        return
    if len(context.args) != 1:
        await update.message.reply_text("âš ï¸ Usage: `/rmove_t <tara_id>`")
    ...
    if remove_normal_tara(tara_id):
        await update.message.reply_text(f"âœ… Removed normal TARA `{tara_id}`.")
        logger.info(f"Removed normal TARA {tara_id} by SUPER_ADMIN {user.id}")
    else:
        await update.message.reply_text(f"âš ï¸ Normal TARA `{tara_id}` not found.")
...
async def group_add_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id != SUPER_ADMIN_ID:
        ...
        return
    if len(context.args) != 1:
        await update.message.reply_text("âš ï¸ Usage: `/group_add <group_id>`")
        logger.warning(f"Incorrect usage of /group_add by SUPER_ADMIN {user.id}")
        return
    try:
        group_id = int(context.args[0])
    except ValueError:
        await update.message.reply_text("âš ï¸ `group_id` must be an integer.")
...
async def tara_link_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 2:
        await update.message.reply_text("âš ï¸ Usage: `/tara_link <tara_id> <group_id>`")
    ...
    try:
        ...
        await update.message.reply_text("âš ï¸ Both `tara_id` and `group_id` must be integers.")
        logger.warning(f"Non-integer arguments provided to /tara_link by SUPER_ADMIN {user.id}")
        return
    if not group_exists(g_id):
        await update.message.reply_text("âš ï¸ Group not added.")
    ...
    await update.message.reply_text(f"âœ… Linked TARA `{tara_id}` to group `{g_id}`.")
...
async def show_groups_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not groups_data:
        await update.message.reply_text("âš ï¸ No groups added.")
    ...
    try:
        ...
        await update.message.reply_text("âš ï¸ An error occurred while sending the groups information.")
...
async def info_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == SUPER_ADMIN_ID or is_global_tara(user_id):
        ...
        if not linked_groups:
            await update.message.reply_text("âš ï¸ No linked groups or permission.")
        ...
        await update.message.reply_text("âš ï¸ You don't have permission to view warnings.")
    ...
    if not rows:
        await update.message.reply_text("âš ï¸ No warnings found.")
    ...
    try:
        ...
        await update.message.reply_text("âš ï¸ An error occurred while sending the warnings information.")
...
async def test_arabic_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        await update.message.reply_text("âš ï¸ Usage: `/test_arabic <text>`")
        return
