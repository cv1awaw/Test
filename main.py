...
async def handle_private_message_for_group_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == SUPER_ADMIN_ID and user.id in pending_group_names:
        ...
        if group_name:
            ...
            await message.reply_text(f"✅ Group name for `{g_id}` set to: *{group_name}*", parse_mode='Markdown')
            logger.info(f"Group name for {g_id} set to {group_name} by SUPER_ADMIN {user.id}")
        else:
            await message.reply_text("⚠️ Group name cannot be empty. Please try `/group_add` again.", parse_mode='Markdown')
            logger.warning(f"Empty group name received from SUPER_ADMIN {user.id} for group {g_id}")
    else:
        await message.reply_text("⚠️ No pending group to set name for.", parse_mode='Markdown')
...
async def set_warnings_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) != 2:
        await update.message.reply_text("⚠️ Usage: `/set <user_id> <number>`", parse_mode='Markdown')
    ...
    try:
        ...
        await update.message.reply_text("⚠️ Both `user_id` and `number` must be integers.", parse_mode='Markdown')
        logger.warning(f"Non-integer arguments provided to /set by SUPER_ADMIN {user.id}")
        return
    if new_warnings < 0:
        await update.message.reply_text("⚠️ Number of warnings cannot be negative.", parse_mode='Markdown')
    ...
    await update.message.reply_text(f"✅ Set `{new_warnings}` warnings for user ID `{target_user_id}`.", parse_mode='Markdown')
...
async def tara_g_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 1:
        await update.message.reply_text("⚠️ Usage: `/tara_G <admin_id>`", parse_mode='Markdown')
        logger.warning(f"Incorrect usage of /tara_G by SUPER_ADMIN {user.id}")
        return
    try:
        new_admin_id = int(context.args[0])
    except ValueError:
        await update.message.reply_text("⚠️ `admin_id` must be an integer.", parse_mode='Markdown')
        logger.warning(f"Non-integer admin_id provided to /tara_G by SUPER_ADMIN {user.id}")
        return
    add_global_tara(new_admin_id)
    await update.message.reply_text(f"✅ Added global TARA admin `{new_admin_id}`.", parse_mode='Markdown')
...
async def remove_global_tara_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id != SUPER_ADMIN_ID:
        ...
        return
    if len(context.args) != 1:
        await update.message.reply_text("⚠️ Usage: `/rmove_G <tara_id>`", parse_mode='Markdown')
        logger.warning(f"Incorrect usage of /rmove_G by SUPER_ADMIN {user.id}")
        return
    try:
        tara_id = int(context.args[0])
    except ValueError:
        await update.message.reply_text("⚠️ `tara_id` must be an integer.", parse_mode='Markdown')
        logger.warning(f"Non-integer tara_id provided to /rmove_G by SUPER_ADMIN {user.id}")
        return

    if remove_global_tara(tara_id):
        await update.message.reply_text(f"✅ Removed global TARA `{tara_id}`.", parse_mode='Markdown')
        logger.info(f"Removed global TARA {tara_id} by SUPER_ADMIN {user.id}")
    else:
        await update.message.reply_text(f"⚠️ Global TARA `{tara_id}` not found.", parse_mode='Markdown')
...
async def tara_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id != SUPER_ADMIN_ID:
        ...
        return
    if len(context.args) != 1:
        await update.message.reply_text("⚠️ Usage: `/tara <tara_id>`", parse_mode='Markdown')
    ...
    await update.message.reply_text(f"✅ Added normal TARA `{tara_id}`.", parse_mode='Markdown')
...
async def remove_normal_tara_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id != SUPER_ADMIN_ID:
        ...
        return
    if len(context.args) != 1:
        await update.message.reply_text("⚠️ Usage: `/rmove_t <tara_id>`", parse_mode='Markdown')
    ...
    if remove_normal_tara(tara_id):
        await update.message.reply_text(f"✅ Removed normal TARA `{tara_id}`.", parse_mode='Markdown')
        logger.info(f"Removed normal TARA {tara_id} by SUPER_ADMIN {user.id}")
    else:
        await update.message.reply_text(f"⚠️ Normal TARA `{tara_id}` not found.", parse_mode='Markdown')
...
async def group_add_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id != SUPER_ADMIN_ID:
        ...
        return
    if len(context.args) != 1:
        await update.message.reply_text("⚠️ Usage: `/group_add <group_id>`", parse_mode='Markdown')
        logger.warning(f"Incorrect usage of /group_add by SUPER_ADMIN {user.id}")
        return
    try:
        group_id = int(context.args[0])
    except ValueError:
        await update.message.reply_text("⚠️ `group_id` must be an integer.", parse_mode='Markdown')
        logger.warning(f"Non-integer group_id provided to /group_add by SUPER_ADMIN {user.id}")
        return

    if group_exists(group_id):
        await update.message.reply_text("⚠️ Group already added.", parse_mode='Markdown')
...
async def rmove_group_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id != SUPER_ADMIN_ID:
        ...
        return
    if len(context.args) != 1:
        await update.message.reply_text("⚠️ Usage: `/rmove_group <group_id>`", parse_mode='Markdown')
    ...
    conn.close()
    if changes > 0:
        await update.message.reply_text(f"✅ Removed group `{group_id}` from registration.", parse_mode='Markdown')
        logger.info(f"Removed group {group_id} by SUPER_ADMIN {user.id}")
    else:
        await update.message.reply_text(f"⚠️ Group `{group_id}` not found.", parse_mode='Markdown')
...
async def tara_link_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 2:
        await update.message.reply_text("⚠️ Usage: `/tara_link <tara_id> <group_id>`", parse_mode='Markdown')
    ...
    try:
        ...
        await update.message.reply_text("⚠️ Both `tara_id` and `group_id` must be integers.", parse_mode='Markdown')
        logger.warning(f"Non-integer arguments provided to /tara_link by SUPER_ADMIN {user.id}")
        return
    if not group_exists(g_id):
        await update.message.reply_text("⚠️ Group not added.", parse_mode='Markdown')
    ...
    await update.message.reply_text(f"✅ Linked TARA `{tara_id}` to group `{g_id}`.", parse_mode='Markdown')
...
async def show_groups_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not groups_data:
        await update.message.reply_text("⚠️ No groups added.", parse_mode='Markdown')
    ...
    try:
        ...
        await update.message.reply_text("⚠️ An error occurred while sending the groups information.", parse_mode='Markdown')
...
async def bypass_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id != SUPER_ADMIN_ID:
        ...
        return
    if len(context.args) != 1:
        await update.message.reply_text("⚠️ Usage: `/bypass <user_id>`", parse_mode='Markdown')
        logger.warning(f"Incorrect usage of /bypass by SUPER_ADMIN {user.id}")
        return
    try:
        target_user_id = int(context.args[0])
    except ValueError:
        await update.message.reply_text("⚠️ `user_id` must be an integer.", parse_mode='Markdown')
        logger.warning(f"Non-integer user_id provided to /bypass by SUPER_ADMIN {user.id}")
        return
    add_bypass_user(target_user_id)
    await update.message.reply_text(f"✅ User `{target_user_id}` has been added to bypass warnings.", parse_mode='Markdown')
...
async def unbypass_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id != SUPER_ADMIN_ID:
        ...
        return
    if len(context.args) != 1:
        await update.message.reply_text("⚠️ Usage: `/unbypass <user_id>`", parse_mode='Markdown')
    ...
    if remove_bypass_user(target_user_id):
        await update.message.reply_text(f"✅ User `{target_user_id}` has been removed from bypass warnings.", parse_mode='Markdown')
        logger.info(f"Removed user {target_user_id} from bypass list by SUPER_ADMIN {user.id}")
    else:
        await update.message.reply_text(f"⚠️ User `{target_user_id}` was not in the bypass list.", parse_mode='Markdown')
...
async def info_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == SUPER_ADMIN_ID or is_global_tara(user_id):
        ...
        if not linked_groups:
            await update.message.reply_text("⚠️ No linked groups or permission.", parse_mode='Markdown')
        ...
        await update.message.reply_text("⚠️ You don't have permission to view warnings.", parse_mode='Markdown')
    ...
    if not rows:
        await update.message.reply_text("⚠️ No warnings found.", parse_mode='Markdown')
    ...
    try:
        ...
        await update.message.reply_text("⚠️ An error occurred while sending the warnings information.", parse_mode='Markdown')
...
async def get_id_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type in ["group", "supergroup"]:
        ...
        await update.message.reply_text("⚠️ This command can only be used in groups.", parse_mode='Markdown')
...
async def test_arabic_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        await update.message.reply_text("⚠️ Usage: `/test_arabic <text>`", parse_mode='Markdown')
        return
